...
knitr::opts_chunk$set(echo = TRUE, cache=T, warning=FALSE, message=FALSE)
...
gradient.rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col=btor, gradient="y")
...
# PLOT
par(mfcol=c(1,2), mar=c(3,3,1.5,1), mgp=c(1.5,0.3,0), xpd=T, tcl=-0.3)

temp22 <- subset(df2, temp=="22")
plot(NA, xlim=c(0,8), ylim=c(0,1), data=temp22, yaxt="n", yaxs="i",
     xlab="Months", ylab="Proportion clade D")
axis(side = 2, at=c(0,1))
mtext(side=3, "a) 22°C", adj=0, font=4)
gradient.rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4],
              col=smoothColors("cornflowerblue", 100, "coral"), gradient="y")
for (pos2 in split(temp22, temp22$pos2)) with(pos2, {
  arrows(time[-1], asymp.LCL[-1], time[-1], asymp.UCL[-1], col="black", code=3, angle=90, length=0.03)
  points(prob ~ time, col="black", type="o", pch=c(24,23,22,25)[pos2], bg="white", cex=1.2)
})
points(2, 0.79, pch="*", cex=2, font=1)
points(3, 0.43, pch="*", cex=2, font=1)

temp29 <- subset(df2, temp=="29")
plot(NA, xlim=c(0,8), ylim=c(0,1), data=temp29, yaxt="n",
     xlab="Months", ylab="Proportion clade D")
axis(side = 2, at=c(0,1))
mtext(side=3, "b) 29°C", adj=0, font=4)
gradient.rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4],
              col=smoothColors("cornflowerblue", 100, "coral"), gradient="y")
...
points(2, 0.77, pch="*", cex=2, font=1)
...
```{r DDswaps, eval = FALSE, include = FALSE, fig.height=8, fig.width=3}
# Look at DD swaps
DDswap <- mcswap %>% anti_join(CD)

# Fit glm (quasibinomial) with time as a factor and host genotype as factor
DDswap$geno <- substr(DDswap$core, 1, 1)
mod <- glm(propD ~ factor(temp) * factor(time) * geno, data=DDswap %>% filter(time==0), family="quasibinomial")
# Get fitted responses for each group
lsm <- lsmeans(mod, specs=c("temp", "time", "geno"), type="response")
xyplot(prob ~ time | temp + geno, data=data.frame(summary(lsm)), type="o")

# Genotype 8 goes back to C-dominance, others not so much. What was starting %C in geno8?
DDswap %>% filter(time==0) %>% group_by(geno) %>% summarise(meanD=mean(propD))


# Plot ending propD vs. starting propD for each core
sdf <- DDswap %>% select(time, swap, pos, core, temp, propD, geno) %>% spread(time, propD) %>% rename(propD.0=`0`, propD.8=`8`)

with(subset(sdf, temp=="22"), plot(propD.8 ~ propD.0, ylim=c(0,1), xlim=c(0.8,1),
               col=factor(geno)))

with(subset(sdf, temp=="29"), plot(propD.8 ~ propD.0, ylim=c(0,1), xlim=c(0.8,1),
               col=factor(geno)))

with(subset(sdf, temp=="22"), plot(log10(1-propD.8) ~ log10(1-propD.0),
               col=factor(geno)))
```
