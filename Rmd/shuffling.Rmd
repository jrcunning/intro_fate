...
p <- plot_ly() %>% 
  add_surface(x=seq(0.5, 1, len=10), y=seq(0, 0.15, len=10), z=as.matrix(dat24), 
              cmin=-1, cmax=1, colorscale="Jet", opacity=0.7) %>%
  ...
  layout(scene=list(aspectmode='manual', 
                    ...
                    xaxis=list(title="% Bleaching"),
                    yaxis=list(title="ADH"),
                    zaxis=list(title="Shuffling", range=c(-1,1)),
                    camera=list(eye=list(x=-1.4,y=-1.5,z=0.8))),
         margin = m)
p
...
plotly_IMAGE(p, format = "png", width=600, height=500, out_file = "figures/3d.png")
```
